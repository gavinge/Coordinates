...
namespace JwUtility
{
	 ...
	 */	
	std::string md5wrapper::hashit(const std::string& text)
	{
		MD5::MD5_CTX ctx;
	 ...
	 */	
#ifdef WIN32
	std::string md5wrapper::getHashFromFile(const std::wstring& filename)
	{
		FILE *file;
		MD5::MD5_CTX context;
		...
		//open file
		if ((file = _wfopen (filename.c_str(), L"rb")) == NULL)
		{
			return "-1";
		}
...
#endif
	std::string md5wrapper::getHashFromFile(const std::string& filename)	
	{
		FILE *file;
		MD5::MD5_CTX context;
