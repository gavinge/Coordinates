...
#ifndef CAppRun_INTERFACE_H
#define CAppRun_INTERFACE_H
#ifdef _WIN32
#pragma once
#endif
...
//回调函数指针集合
typedef struct AppRun_CALLBACKFUNC_PTR_SET
{
	DECLARE_DEFAULT_CONSTRUCTOR(AppRun_CALLBACKFUNC_PTR_SET);
	DECLARE_COPY_CONSTRUCTOR(AppRun_CALLBACKFUNC_PTR_SET);
	DECLARE_OPERATOR_EQUAL(AppRun_CALLBACKFUNC_PTR_SET);

	DWORD dwSize;                           //CALLBACKFUNC 结构大小
	///!!这里函数指针描述，请按照顺序描述(加了就站位，不要换顺序或者删除)。否则会出现新老版本函数地址错乱的问题。
	//LPFunCallBackDemo lpFunCallBackDemo;
}AppRun_CALLBACKFUNC_PTR_SET, *LPAppRun_CALLBACKFUNC_PTR_SET;
...
#define  MAX_AppRun_VERSION  1       //接口版本(高)
#define  MIN_AppRun_VERSION  0       //接口版本(低)



///对外的公共接口（自定义+公共Base接口）
class DEF_NO_VTABLE IAppRun
{
public:
	...
	///@return int 成功>0
	STDMETHOD_(int, InitCallBack)(const AppRun_CALLBACKFUNC_PTR_SET & callback_ptr_set) PURE;
	...
	///具体的操作方法
	virtual int addRule_RunApp(const AppRunManagement::RuleRunApp& rule) PURE;
	virtual int updateRule_RunApp(const AppRunManagement::RuleRunApp& rule)PURE;
	virtual int deleteRule_RunApp(const AppRunManagement::RuleRunApp& rule)PURE;
	virtual bool queryRecords_RunApp(std::vector<AppRunManagement::RuleRunApp>& queryResult)PURE;
	virtual bool queryRecords_RunApp(int ruleType, std::vector<AppRunManagement::RuleRunApp>& queryResult)PURE;
};

///dll统一导出接个接口.
//EXTERN_API BOOL API_CALLTYPE CreateInstance(IAppRun**);
