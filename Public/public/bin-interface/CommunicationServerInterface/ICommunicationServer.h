...
#ifndef CCommunicationServer_INTERFACE_H
#define CCommunicationServer_INTERFACE_H

#include "ICommunicationServerNotify.h"
#include "ICommunicationDef.h"
#ifdef _WIN32
#pragma once
#endif
...
//回调函数指针集合
typedef struct CommunicationServer_CALLBACKFUNC_PTR_SET
{
	DECLARE_DEFAULT_CONSTRUCTOR(CommunicationServer_CALLBACKFUNC_PTR_SET);
	DECLARE_COPY_CONSTRUCTOR(CommunicationServer_CALLBACKFUNC_PTR_SET);
	DECLARE_OPERATOR_EQUAL(CommunicationServer_CALLBACKFUNC_PTR_SET);

	DWORD dwSize;                           //CALLBACKFUNC 结构大小
	///!!这里函数指针描述，请按照顺序描述(加了就站位，不要换顺序或者删除)。否则会出现新老版本函数地址错乱的问题。
	//LPFunCallBackDemo lpFunCallBackDemo;
}CommunicationServer_CALLBACKFUNC_PTR_SET, *LPCommunicationServer_CALLBACKFUNC_PTR_SET;
...
#define  MAX_CommunicationServer_VERSION  1       //接口版本(高)
#define  MIN_CommunicationServer_VERSION  0       //接口版本(低)



///对外的公共接口（自定义+公共Base接口）
class DEF_NO_VTABLE ICommunicationServer
{
public:
	...
	///@return int 成功>0
	STDMETHOD_(int, InitCallBack)(const CommunicationServer_CALLBACKFUNC_PTR_SET & callback_ptr_set) PURE;
	...
	//服务初始化
	virtual int InitServer(ICommunicationServerNotify* callbackNotify) PURE;

	//服务开始
	virtual int StartServer() PURE;

	//服务停止
	virtual int StopServer() PURE;

	// 批量给所有客户端发送数据的函数
	virtual bool SendDataToAllClients(const BYTE* pData, int iLength) = 0;

	// 给指定客户端发送数据的函数
	//virtual bool SendDataToClient(const char* strMAC, const BYTE* pData, int iLength) = 0;

	// 给指定客户端发送数据的函数
	virtual bool SendDataToClient(DeliverCommandType commandType,const char* strMAC, const BYTE* pData, int iLength) = 0;
};

///dll统一导出接个接口.
//EXTERN_API BOOL API_CALLTYPE CreateInstance(ICommunicationServer**);
