...
#ifndef __RULE_THREAD_CREATION_DEFINE_H__
#define __RULE_THREAD_CREATION_DEFINE_H__
#ifdef _WIN32
#pragma once
#endif
...
namespace RuleManagement
{
	...
	"ProcessInfoType": 0,
	"ProcessInfo": "*",
	"ExpressInfoType": 0, //0|1
	"ExpressInfo": "mov eax,0",
	"Log": 1,
	...
	*/
	...
	public:
		...
		RuleThreadCreation() {
			memset(this, 0, sizeof(RuleThreadCreation));
		}
		~RuleThreadCreation() {}
		...
		RuleThreadCreation(int rid, int ruleGroupID, int ruleType, int operationCode, int processInfoType, const std::string& processInfo,
			...
			const std::string& remark, int osVersion, int priority, int createTime, int updateTime, int reserved1, int reserved2, int reserved3, const std::string& reserved4, const std::string& reserved5, const std::string& reserved6)
			: RID(rid),
		...
		void toJson(Json::Value& root) const {
			...
			root["ExpressInfoType"] = ExpressInfoType;
			root["ExpressInfo"] = ExpressInfo;
			root["Log"] = Log;
		...
		void fromJson(const Json::Value& root) {
			...
			ExpressInfoType = root.get("ExpressInfoType", 0).asInt();
			ExpressInfo = root.get("ExpressInfo", "").asString();
