...
#ifndef __RULE_REGISTER_PROTECT_DEFINE_H__
#define __RULE_REGISTER_PROTECT_DEFINE_H__
#ifdef _WIN32
#pragma once
#endif
...
namespace RuleManagement
{
	...
	*/
	...
	public:
		...
		RuleRegisterProtect() {
			memset(this, 0, sizeof(RuleRegisterProtect));
		}
		~RuleRegisterProtect() {}
		...
		RuleRegisterProtect(int rid, int rule_group_id, int operation_code, int rule_type, const std::string& process_info, int process_info_type,
			int options, const std::string& registry_path, int inner_ID, int inner_flg, int enabled, int log, const std::string& remark, int os_version, int priority, int createTime, int updateTime, int reserved1, int reserved2, int reserved3, const std::string& reserved4, const std::string& reserved5, const std::string& reserved6)
			: RID(rid),
		...
		void toJson(Json::Value& root) const {
			...
			root["RegistryPath"] = RegistryPath;
		...
		void fromJson(const Json::Value& root) {
			...
			RegistryPath = root.get("RegistryPath", "").asString();
