...
void ThreadRuleDetailWidget::onItemChanged(QTableWidgetItem *item) {
	if (!parentPtr || !item) {
		return;
	}
	if (item->column() == 0) {
		...
		if (pItem)
		{
			...
			if (item->checkState() == Qt::Checked) {
				((CustomRuleWidget*)parentPtr)->EnableRuleInfo(true, RuleType::ThreadRuleType, nCurrentRuleID);
			}
			else {
				((CustomRuleWidget*)parentPtr)->EnableRuleInfo(false, RuleType::ThreadRuleType, nCurrentRuleID);
			}
		}
	}
}

void ThreadRuleDetailWidget::contextMenuEvent(QContextMenuEvent *event)
{
...
int ThreadRuleDetailWidget::RefreshList() {
	return initData();
}
int ThreadRuleDetailWidget::RefreshListByGourpID(int agentGroupID) {
	nCurrentGroupID = agentGroupID;
	return initData();
}

void ThreadRuleDetailWidget::setCustomRuleWidgetPtr(void* parent) {
	parentPtr = parent;
}

long ThreadRuleDetailWidget::getCurrentRuleID() {
	return nCurrentRuleID;
}
int ThreadRuleDetailWidget::initData(){
...
bool ThreadRuleDetailWidget::ClearTable(QTableWidget* tableWidget) {
...
int ThreadRuleDetailWidget::LoadRuleList() {

	//RuleThreadCreation rule_info(0, 25, 14, 0, 0, "*", 0, "mov eax,0", 1, 20, 1, 1, "123123123123123123123123123123", 3);

	std::vector<RuleManagement::RuleThreadCreation> threadCreationsRules;
	RULE_MAN_CONTROLLER->queryThreadCreation(nCurrentGroupID, threadCreationsRules);

	ui->tableWidget->setRowCount(threadCreationsRules.size());

	ui->tableWidget->blockSignals(true);

	for (int row = 0; row < threadCreationsRules.size(); ++row) {

		RuleManagement::RuleThreadCreation rule_info = threadCreationsRules[row];
		//0.状态或者编号
	...
	connect(ui->tableWidget, &QTableWidget::itemChanged, this, &ThreadRuleDetailWidget::onItemChanged);
	return threadCreationsRules.size();
}