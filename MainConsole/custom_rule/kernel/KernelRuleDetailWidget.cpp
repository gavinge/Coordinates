...
void KernelRuleDetailWidget::onItemChanged(QTableWidgetItem *item) {
	if (!parentPtr || !item) {
		return;
	}
	if (item->column() == 0) {
		...
		if (pItem)
		{
			...
			if (item->checkState() == Qt::Checked) {
				((CustomRuleWidget*)parentPtr)->EnableRuleInfo(true, RuleType::KernelRuleType, nCurrentRuleID);
			}
			else {
				((CustomRuleWidget*)parentPtr)->EnableRuleInfo(false, RuleType::KernelRuleType, nCurrentRuleID);
			}
		}
	}
}

void KernelRuleDetailWidget::contextMenuEvent(QContextMenuEvent *event)
{
...
int KernelRuleDetailWidget::RefreshList() {
	return initData();
}
int KernelRuleDetailWidget::RefreshListByGourpID(int agentGroupID) {
	nCurrentGroupID = agentGroupID;
	return initData();
}

void KernelRuleDetailWidget::setCustomRuleWidgetPtr(void* parent) {
	parentPtr = parent;
}

long KernelRuleDetailWidget::getCurrentRuleID() {
	return nCurrentRuleID;
}

int KernelRuleDetailWidget::initData(){
...
bool KernelRuleDetailWidget::ClearTable(QTableWidget* tableWidget) {
...
int KernelRuleDetailWidget::LoadRuleList() {

	//RuleThreadCreation rule_info(0, 25, 14, 0, 0, "*", 0, "mov eax,0", 1, 20, 1, 1, "123123123123123123123123123123", 3);

	std::vector<RuleManagement::RuleKernelProtect> kernelProtectRules;
	RULE_MAN_CONTROLLER->queryKernelProtect(nCurrentGroupID, kernelProtectRules);

	ui->tableWidget->setRowCount(kernelProtectRules.size());

	ui->tableWidget->blockSignals(true);

	for (int row = 0; row < kernelProtectRules.size(); ++row) {

		RuleManagement::RuleKernelProtect rule_info(kernelProtectRules[row]);
		//0.状态或者编号
	...
	connect(ui->tableWidget, &QTableWidget::itemChanged, this, &KernelRuleDetailWidget::onItemChanged);
	return kernelProtectRules.size();
}