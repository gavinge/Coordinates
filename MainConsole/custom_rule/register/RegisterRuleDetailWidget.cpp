...
void RegisterRuleDetailWidget::onItemChanged(QTableWidgetItem *item) {
	if (!parentPtr || !item) {
		return;
	}
	if (item->column() == 0) {
		...
		if (pItem)
		{
			...
			if (item->checkState() == Qt::Checked) {
				((CustomRuleWidget*)parentPtr)->EnableRuleInfo(true, RuleType::RegistryRuleType, nCurrentRuleID);
			}
			else {
				((CustomRuleWidget*)parentPtr)->EnableRuleInfo(false, RuleType::RegistryRuleType, nCurrentRuleID);
			}
		}
	}
}

void RegisterRuleDetailWidget::contextMenuEvent(QContextMenuEvent *event)
{
...
int RegisterRuleDetailWidget::RefreshList() {
	return initData();
}
int RegisterRuleDetailWidget::RefreshListByGourpID(int agentGroupID) {
	nCurrentGroupID = agentGroupID;
	return initData();
}

void RegisterRuleDetailWidget::setCustomRuleWidgetPtr(void* parent) {
	parentPtr = parent;
}

long RegisterRuleDetailWidget::getCurrentRuleID() {
	return nCurrentRuleID;
}

int RegisterRuleDetailWidget::initData(){
...
bool RegisterRuleDetailWidget::ClearTable(QTableWidget* tableWidget) {
...
int RegisterRuleDetailWidget::LoadRuleList() {

	std::vector<RuleManagement::RuleRegisterProtect> registgerProtectRules;
	RULE_MAN_CONTROLLER->queryRegisterProtect(nCurrentGroupID, registgerProtectRules);

	ui->tableWidget->setRowCount(registgerProtectRules.size());

	ui->tableWidget->blockSignals(true);

	for (int row = 0; row < registgerProtectRules.size(); ++row) {
		RuleManagement::RuleRegisterProtect rule_info(registgerProtectRules[row]);
		...
		QTableWidgetItem *textItem4 = new QTableWidgetItem(QString::fromLocal8Bit(strPrivilege.c_str()));
	...
	connect(ui->tableWidget, &QTableWidget::itemChanged, this, &RegisterRuleDetailWidget::onItemChanged);

	return registgerProtectRules.size();
}
