#include "NetWorkRuleDetailWidget.h"
#include "ui_NetWorkRuleDetailWidget.h"
#include "common/GlobalDefineUtility.h"
#include "common/ItemDelegate.h"

NetWorkRuleDetailWidget::NetWorkRuleDetailWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::NetWorkRuleDetailWidget),
	parentPtr(NULL),
	nCurrentRuleID(0),
	nCurrentGroupID(1)
{
...
NetWorkRuleDetailWidget::NetWorkRuleDetailWidget(QWidget *parent,RuleViewModel model) :
    QWidget(parent),
    ui(new Ui::NetWorkRuleDetailWidget),
    ruleViewModel(model),
	parentPtr(NULL),
...
NetWorkRuleDetailWidget::~NetWorkRuleDetailWidget()
{
    delete ui;
}


void NetWorkRuleDetailWidget::onItemChanged(QTableWidgetItem *item) {
	if (!parentPtr || !item) {
		return;
	}
	if (item->column() == 0) {
		...
		if (pItem)
		{
			...
			if (item->checkState() == Qt::Checked) {
				((CustomRuleWidget*)parentPtr)->EnableRuleInfo(true, RuleType::NetworkRuleType, nCurrentRuleID);
			}
			else {
				((CustomRuleWidget*)parentPtr)->EnableRuleInfo(false, RuleType::NetworkRuleType, nCurrentRuleID);
			}
		}
	}
}

void NetWorkRuleDetailWidget::contextMenuEvent(QContextMenuEvent *event)
{
...
int NetWorkRuleDetailWidget::RefreshList() {
	return initData();
}
int NetWorkRuleDetailWidget::RefreshListByGourpID(int agentGroupID) {
	nCurrentGroupID = agentGroupID;
	return initData();
}

void NetWorkRuleDetailWidget::setCustomRuleWidgetPtr(void* parent) {
	parentPtr = parent;
}

long NetWorkRuleDetailWidget::getCurrentRuleID() {
	return nCurrentRuleID;
}

int NetWorkRuleDetailWidget::initData(){
    ...
    headerLabels << QString::fromLocal8Bit(colum1header.c_str())
                 ...
                 << QString::fromLocal8Bit("目标IP")
                 << QString::fromLocal8Bit("目标端口")
                 << QString::fromLocal8Bit("协议")
...
bool NetWorkRuleDetailWidget::ClearTable(QTableWidget* tableWidget) {
...
int NetWorkRuleDetailWidget::LoadRuleList() {
	std::vector<RuleManagement::RuleNetworkProtect> networkProtectRules;
	RULE_MAN_CONTROLLER->queryNetworkProtect(nCurrentGroupID, networkProtectRules);

	ui->tableWidget->setRowCount(networkProtectRules.size());

	ui->tableWidget->blockSignals(true);

	for (int row = 0; row < networkProtectRules.size(); ++row) {

		RuleManagement::RuleNetworkProtect rule_info(networkProtectRules[row]);
		//0.状态或者编号
		...
		//3.目标IP
		QTableWidgetItem *textItem3 = new QTableWidgetItem(QString::fromLocal8Bit(rule_info.IP.c_str()));
		...
		//4.目标端口
		QTableWidgetItem *textItem4 = new QTableWidgetItem(QString::fromLocal8Bit(rule_info.Port.c_str()));
		textItem4->setTextAlignment(Qt::AlignCenter);
		ui->tableWidget->setItem(row, 4, textItem4);

		//5.协议
		QTableWidgetItem *textItemx = new QTableWidgetItem("");
	...
	connect(ui->tableWidget, &QTableWidget::itemChanged, this, &NetWorkRuleDetailWidget::onItemChanged);
	return networkProtectRules.size();
}